...
const getCourses = asyncHandler (async (req, res, next) => {
    ...
    // first get the courses from only one particular bootcamp, which requires just a simple query
    if (req.params.bootcampId){
        const courses = await Course.find({bootcamp: req.params.bootcampId});
        return res.status(200).json({success: true, count: courses.length, data: courses });
    } else {
        // The second route gets all courses from all bootcamps which requires a more advanced query and pagination functionality
